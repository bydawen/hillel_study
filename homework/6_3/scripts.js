...
let usersSumBalance = 0;
let arrUsersPhones = [];

for (let i= 0; i < users.length; i++) {
  if (users[i].balance > 2000) {
    arrUsersPhones.push(users[i].phone);
  }

  usersSumBalance += users[i].balance;
}

document.write(`Телефони баланс яких меньше ніж 2000: <br> ${arrUsersPhones.join("<br>")} <br>`);
document.write(`<br> Сума усіх балансів: <br> ${usersSumBalance}`);
